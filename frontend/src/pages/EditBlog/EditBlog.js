...
function EditBlog() {
    ...
    const handleSave = async () => {
        try {
            // Update the blog content and add the new tag
            await axios.put(`http://localhost:8080/api/update-blog/${userId}/${title}`,
                { content: blogContent, newTag }, // Sending newTag along with content
